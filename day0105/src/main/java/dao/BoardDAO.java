...
public class BoardDAO {
	...
	public int myTotalRecord(String writer) {
		int re =0;
...
//		String sql = "select no,title,writer,pwd,content,regdate,hit,fname,ip,b_ref,b_level,b_step from (select rownum n,no,title,writer,pwd,content,regdate,hit,fname,ip,b_ref,b_level,b_step from(select * from board order by b_ref desc, b_step))a where a.n  between ? and ?";
		String sql = "select no,title,writer,pwd,content,regdate,hit,fname,ip,b_ref,b_level,b_step from (select rownum n,no,title,writer,pwd,content,regdate,hit,fname,ip,b_ref,b_level,b_step from(select * from board ";
		
		if(writer !=null) {
			sql+="where writer = '"+writer+"'";
			totalRecord = myTotalRecord(writer);
			totalPage = (int)Math.ceil(totalRecord*1.0/pageSIZE);

			
		}
		sql+= " order by b_ref desc, b_step))a where a.n  between ? and ?";
		try {
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql);
